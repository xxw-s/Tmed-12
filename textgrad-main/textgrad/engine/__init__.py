...
def _normalize_engine_name(engine_name: str) -> str:
    ...
    if normalized.startswith("vllm-"):
        normalized = normalized[len("vllm-"):]
    return normalized


def _check_if_multimodal(engine_name: str):
    normalized = _normalize_engine_name(engine_name)
    if any(normalized == name.lower() for name in __MULTIMODAL_ENGINES__):
        return True
    return any(keyword in normalized for keyword in __MULTIMODAL_KEYWORDS__)
