...
class VllmServer(EngineLM, CachedEngine):
    ...
    def _generate_from_single_prompt(
        ...
        sys_prompt_arg = system_prompt if system_prompt else self.system_prompt
        # -------------------------- 新增：动态计算输入token数和max_tokens --------------------------
        # 1. 拼接所有输入文本（system prompt + user prompt）
        input_text = f"{sys_prompt_arg}\n{prompt}"
        # 2. 统计输入token数（忽略特殊token，用add_special_tokens=False避免多算）
        input_token_count = len(self.tokenizer.encode(input_text, add_special_tokens=False))
        # 3. 动态计算允许的max_tokens：模型最大长度 - 输入token数 - 10（预留余量）
        dynamic_max_tokens = self.model_max_length - input_token_count - 10
        # 4. 兜底：避免dynamic_max_tokens过小（至少10，保证能生成内容），且不超过原max_tokens参数
        dynamic_max_tokens = max(dynamic_max_tokens, 10)
        dynamic_max_tokens = min(dynamic_max_tokens, self.max_tokens, max_tokens)
        # 5. 警告：若输入token数接近模型上限，提示可能生成不完整
        if input_token_count > self.model_max_length - 100:
            print(f"⚠️ 警告：输入文本过长（{input_token_count} tokens），生成内容可能不完整（模型上限{self.model_max_length} tokens）")
        # -------------------------------------------------------------------------
        # cache_or_none = self._check_cache(sys_prompt_arg + prompt)
        # if cache_or_none is not None:
        #     return cache_or_none
...
#-------------------------用动态计算的结果-----------------
    ...
    def _generate_from_multiple_input(
        ...
        # -------------------------- 新增：动态计算多模态输入的max_tokens --------------------------
        # 1. 提取formatted_content中的所有文本（忽略图片）
        input_text_parts = [sys_prompt_arg]  # 先加system prompt
        for item in formatted_content:
            if item["type"] == "text":  # 只处理文本，图片不占token
                input_text_parts.append(item["text"])
        input_text = "\n".join(input_text_parts)  # 拼接所有文本
        # 2. 统计输入token数
        input_token_count = len(self.tokenizer.encode(input_text, add_special_tokens=False))
        # 3. 动态计算max_tokens（逻辑同纯文本）
        dynamic_max_tokens = self.model_max_length - input_token_count - 10
        dynamic_max_tokens = max(dynamic_max_tokens, 10)
        dynamic_max_tokens = min(dynamic_max_tokens, self.max_tokens, max_tokens)
        # 4. 警告提示
...
#-------------------------用动态计算的结果-----------------
        ...
        if n == 1 and cache_or_none is None:
            # self._save_cache(cache_key, response_text)
            pass
        return response_text
