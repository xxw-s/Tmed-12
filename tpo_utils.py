...
def run_test_time_training_tpo(
    ...
    def _score_and_select(
        ...
        for resp in responses:
            ...
            # 移除各种可能的前缀
            if candidate.startswith("INDEX-1<SEP>"):
                candidate = candidate[len("INDEX-1<SEP>") :].strip()
            if candidate.startswith("Assistant:"):
                candidate = candidate[len("Assistant:") :].strip()
            if candidate.startswith("Human:"):
                candidate = candidate[len("Human:") :].strip()
            # 过滤掉错误消息
            if candidate and "无法生成有效响应" not in candidate:
    ...
    for resp in init_responses:
        if isinstance(resp, str) and resp.strip() and resp.strip() != "无法生成有效响应":
            valid_responses.append(resp)
    
    if not valid_responses:
        # 尝试重新生成
        print("⚠️ 初始响应无效，尝试重新生成...")
        init_responses = _ensure_list(
            llm_engine(prepared["llm_input"], system_prompt=system_prompt, temperature=0.8, **gen_params)
        )
        # 再次过滤
        valid_responses = []
        for resp in init_responses:
        ...
        if not valid_responses:
            # 如果仍然没有有效响应，使用一个默认响应继续
            print("⚠️ 无法生成有效响应，使用默认响应继续...")
            valid_responses = ["这是一个初始响应。请提供更多信息以获取更准确的回答。"]
    
    chosen_resp_text, rej_resp_text = _score_and_select(
        valid_responses, iteration_index=-1
    )
...
# 修改迭代循环部分，正确使用continue
    for iteration in range(max_iters):
        ...
        try:
            ...
            if not valid_responses:
                # 如果没有有效响应，使用当前响应继续
                valid_responses = [response.get_value()]
            ...
            chosen_resp_text, rej_resp_text = _score_and_select(
                valid_responses, iteration_index=iteration
            )

            if not chosen_resp_text:
                # 在循环内使用continue是正确的
                continue

            # 6.5) Update the variable's content
            response.set_value(chosen_resp_text)
            last_rejected = rej_resp_text if rej_resp_text else last_rejected

            # 6.6) Update the loss function if needed
            if tpo_mode == "tpo" and rej_resp_text:
                evaluation_sys_text = EVALUATION_SYS_TEMPLATE.format(
                    query=prepared["prompt"],
                    rejected_response=rej_resp_text,
                )
