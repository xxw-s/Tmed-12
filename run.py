...
# Ensure the local TextGrad package is available before importing.
_REPO_ROOT = Path(__file__).resolve().parent
_TEXTGRAD_ROOT = _REPO_ROOT / "textgrad-main"
if (_TEXTGRAD_ROOT / "textgrad").is_dir():
    sys.path.insert(0, str(_TEXTGRAD_ROOT))
...
def _load_existing_results(path: str) -> Tuple[List[Dict[str, Any]], Set[str], bool]:
    try:
        with open(path, "r", encoding="utf-8") as f:
            raw_results = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return [], set(), False  # 移除 processed_prompts
    ...
    for entry in raw_results:
        if isinstance(entry, dict) and {"sample_id", "scores"}.issubset(entry.keys()):
        ...
        if isinstance(entry, dict):
            needs_flush = True
            keys = [key for key in entry.keys() if isinstance(key, str) and "<SEP>" in key]
            prompt_text = keys[0].split("<SEP>")[1] if keys else ""
            if prompt_text:
                sample_id = prepare_sample(prompt_text)["id"]
                processed_prompts.add(prompt_text)
            else:
                sample_id = f"legacy-{len(normalized_results)}"
            processed_ids.add(sample_id)
    ...
    return normalized_results, processed_ids, needs_flush    #不再跟踪 prompt


def _update_cache_sets(result: Dict[str, Any], ids: Set[str]) -> None:  # 移除 prompts 参数
    sample_id = result.get("sample_id")
    if isinstance(sample_id, str):
        ids.add(sample_id)
    #prompt = result.get("prompt")
    #if isinstance(prompt, str):
        #prompts.add(prompt)  # 移除对 prompt 的更新
...
if __name__ == "__main__":
    ...
    prepared_samples = [prepare_sample(sample, idx) for idx, sample in enumerate(raw_data)]
    ...
    existing_results, processed_ids, needs_flush = _load_existing_results(out_path) # 移除对 prompt 的跟踪
    if needs_flush:
    ...
    for i in range(0, len(prepared_samples), chunk_size):
        results, processed_ids, needs_flush = _load_existing_results(out_path)  # 移除对 prompt 的跟踪

        start_idx, end_idx = i, min(i + chunk_size, len(prepared_samples))
        data_chunk = prepared_samples[start_idx:end_idx]

        # Parallel execution
        futures: List[concurrent.futures.Future] = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_threads) as executor:
            for sample in tqdm(data_chunk, desc="Processing Chunk", ncols=80):
                sample_id = sample["id"]
                prompt = sample["prompt"]
                if sample_id in processed_ids: #or prompt in processed_prompts:
                    continue
            ...
            for future in tqdm(concurrent.futures.as_completed(futures),
                ...
                if isinstance(answer, dict):
                    collected_results.append(answer)

        # Save intermediate results
        if collected_results:
            results.extend(collected_results)
            for res in collected_results:
                _update_cache_sets(res, processed_ids)  # 移除对 prompt 的跟踪

        if collected_results or needs_flush or not os.path.exists(out_path):
            with open(out_path, "w", encoding="utf-8") as f:
